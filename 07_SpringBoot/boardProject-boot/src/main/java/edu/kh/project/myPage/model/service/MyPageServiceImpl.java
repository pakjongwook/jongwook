...
public class MyPageServiceImpl implements MyPageService {
	...
	@Value("${my.member.webpath}") // properties 값을 꺼내기 위해서 사용
	private String webPath;
	
	@Value("${my.member.location}")
	private String filePath;
	
	@Autowired // MyPageDAO에 의존성 주입(DI)
	private MyPageMapper mapper;
	
	
	@Autowired
//	private BCryptPasswordEncoder bcrypt; 의존성 주입(DI)
...
//	@Transactional // UnChecked Exception 발생 시 rollback
	...
	public int secession(String memberPw, int memberNo) {
		...
		if(bcrypt.matches(memberPw, encPw)) {
			return mapper.secession(memberNo);  // 몇번 회원 번호 탈퇴할래 
		}
	...
	public int updateProfile(MultipartFile profileImage, Member loginMember) throws IllegalStateException, IOException {
		...
		int result = mapper.updateProfileImage(loginMember); // loginMember 회원번호.. 다담겨있음
		...
		if(result >0) { // 성공
			...
			if(rename != null ) {
				...
				profileImage.transferTo(new File(filePath + rename)); // 메모리 -> 하드디스크
			}
