...
public class BoardServiceImpl implements BoardService {
	...
	// alt + shift + r -> 모두 바뀌는 것 바꾸고 싶은 텍스트 클릭한 상태에서
	...
	public Map<String, Object> selectBoardList(int boardCode, int cp) {
		...
		int listCount = mapper.getListCount(boardCode); // boardCode 어떤 게시판인지 전달하기 때문
	...
	// 좋아요 처리 서비스
	@Transactional(rollbackFor = Exception.class) // insert,delete 구문을 쓰기 때문
	@Override
	public int like(Map<String, Integer> paramMap) {

		int result = 0;
		if (paramMap.get("check") == 0) { // 좋아요 상태 X (JS의 파일확인)
	...
	// 조회수 증가 서비스
	@Transactional(rollbackFor = Exception.class) // 최소 2개 이상의 DML 구문을 쓸 때 사용
	@Override
	...
	public Map<String, Object> selectBoardList(Map<String, Object> paramMap, int cp) {
		...
		List<Board> boardList = mapper.selectSearchBoardList(paramMap, rowBounds); // rowBounds 는 두번째로 mapper로 전달
