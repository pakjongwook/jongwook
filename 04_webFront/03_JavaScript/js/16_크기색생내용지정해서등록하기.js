const size = document.getElementById('size');
const color = document.getElementById('color');
const apply = document.getElementById('apply');
const content = document.getElementById('content');
const registation = document.getElementById('registation');
const result = document.getElementById('result');


// 적용 (apply) 버튼 클릭 시 size, color에 작성된 값을 content의 css(style)적용
apply.addEventListener('click', () => {

    content.style.color = color.value;
    content.style.fontSize = size.value + 'px';
});

// 등록 버튼 클릭 시 요소를 생성하여 content에 작성된 내용, style  적용 후  
// result에 추가
registation.addEventListener('click', () => {
    const p = document.createElement('p');
    p.innerText = content.value; // content 작성된 내용을 p의 내용으로 대입

    const contentStyle =  content.getAttribute("style"); // content style얻어오기
    p.setAttribute("style", contentStyle); // content의 style을 p에 세팅
    result.append(p); // result에 p 추가

});