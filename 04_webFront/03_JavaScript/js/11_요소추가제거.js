...
document.getElementById("calc").addEventListener("click", ()=>{
    ...
    // 계산 버튼이 클릭되는 시점의 .in input 태그들을 모두 읽어옴 => 칸이 추가되어도 다 계산되어짐
    // ? -> for문을 돌렸기 때문
    ...
    for (let i = 0; i < list.length; i++) {
        ...
        // for(let input of list){ sum += Number(input.value); }
        // console.log(list[i]);
        
        // HTML 작성된 모든 내용, 값은 string(문자열)!!!
        sum += Number(list[i].value); // input 태그의 vaule
    }
...
document.getElementById("add").addEventListener("click", ()=>{
    ...
    // 요소에 class 추가하기 : 요소.classList.add("클래스명")
    // 요소에 class 삭제하기 : 요소.classList.remove("클래스명")
    ...
    // span에 &tiems; 내용 추가(innerHTML 사용)
...
document.querySelector(".remove-row").addEventListener("click",(e) => {
    ...
    console.log(e.target); // 이벤트가 발생한 요소--> x버튼(클릭된 버튼)

    console.log(e.target.parentElement); // 이벤트가 발생한 부모 요소(.row)
