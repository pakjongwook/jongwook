...
function accessName(){
    ...
    // 2) check된 체크박스의 value를 저장할 변수
    //  + 개수를 세기 위한 변수 선언
    var result = ""; // result 는 빈칸
    var count = 0;
...
function accessTagName(){
    ...
    for(var i = 0; i < arr.length; i++){
        ...
        //  arr[i].style.color = arr[i].innerText arr[i] 번째 올때 마다 innerText 값을 변경
...
// input 태그에 enter가 눌러졌을 때
function inputEnter(){
    
    // console.log("window.event.key"); // 현재 눌러진 키를 반환
    // 엔터 입력 시 readValue() 함수를 호출
