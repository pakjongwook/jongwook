...
public class LoginController extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		String id = req.getParameter("id"); // parameter( vscode에서 쓴 것과 같은 것을 써야함 )
			// 변수명은 아무거나 써도 상관없음
		String Pw = req.getParameter("pw");
		
		// id/pw 가 user01/pass01 과 일치하면 "로그인 성공"
		...
		// ---------------------------------------------------------
		// 기존에 req에 없던 값(message)을 속성(attribute)으로 추가 [ 이름값] - >[request에 한테 변수하나를 만들었구나]
		// -> Map(key, value) 형식으로 작성
