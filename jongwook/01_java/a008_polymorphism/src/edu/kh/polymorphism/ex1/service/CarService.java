...
import edu.kh.polymorphism.ex1.dto.*;  // *; 표시 => import 부문 All  ctrl + d  줄삭제 
...
public class CarService {
	...
	// 부모 참조 변수 = 자식 객체
		Car c2 = new Truck();  // 오류 발생시 import 부문 생각 ctrl + 스페이스바
	...
	// 부모 타입 참조 변수로 참조 시 
	// 자식 객체의 부모 부분만 참조 가능  자식이 Truck 부모참조변수 내에서는 Truck 없음
...
//		c1.getDiscountOffer();		
	...
	public void ex3() {
		...
		for(Car c : carList) {
			...
			// ex) LightCar 객체를 Truck 참조변수가 참조하려고 할 때  {다형성 상속이 되어있을때만}
...
//		printCar(new Truck());
		...
		// 차량 정보를 출력하는 메서드
		public void printCar(Car c) { // 부모 참조 변수 
			String type = null;
	...
	public void ex5() {
		...
		Car c = createCar(sc.nextInt()); // 다음 입력된 정수를 아래쪽에 있는 createrCar 매개변수로 보내라
