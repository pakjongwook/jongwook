...
public class ForEx {
	...
	/*
	 * for 문 - 끝이 정해져 있는 경우에 사용하는 반복문 (== 반복 횟수가 지정되어 있는 경우 사용)
	 * 
	 * - 작성법 int a = 1; a < 3 for( 초기식[1] ; 조건식[2][5] ; [4]증감식 ){ [3][6]조건식이 true일 때
	 * 반복 수행할 코드
	 * 
	 * 
	 * }
	 ...
	 * 
	 * - 증감식 : for문이 한 번 반복을 수행할 때 마다 마지막에 특정 값을 증가 또는 감소시키는 식
	 * 
	...
	public void ex17() {
		...
		for (int dan = 2; dan <= 9; dan++) {
			...
			for (int num = 1; num < 9; num++) {
				System.out.printf("%d x%2d = %d ", dan, num, dan * num);
			}
	...
	// 중첩 반복문 응용 사용법4
		...
		// 카운트(개수 세기)
				...
				public void ex22(){
					...
					int count = 1; // 숫자를 세기위한 변수 선언(초기화선언)
					
					for(int row=1; row<=3; row++) {// 3행
						
						for(int col=1; col<=4; col++) { // 4열
							
							System.out.printf("%3d" ,count); // 제일 마지막에 계산
							count++; // 출력 후 1증가(후위 연산)
						}
						System.out.println();// 개행
					}
