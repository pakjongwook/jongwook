...
import edu.kh.objactarray.service.StudentService;
...
public class StudentView {
	...
	// StudentService 참조할 참조 변수 => 새롭게 만들어진 객체의 주소를 참조하겠다.
	...
	public void displayMenu() {
		...
		do {
			...
			case 3 : selcetIndex(); break;
			case 4 : selcetName();	 break;
	...
	private void addStudent() {
		...
		if(result) // 조건식 true false) true 인 경우 == 학생 추가 성공 
		{		System.out.println(name + "학생이 추가되었습니다!.");
		 
		}else { // false 인경우 == 학생 추가 실패
	...
	private void selectAll() {
		...
		// ***********************************
		// 매개변수, 반환값으로 참조변수가 사용될 경우  // 참조변수 :stack 영역에 공간을 생성하고 heap 영역에 새로운 공간을 생성해 객체 주소를 찾아갈 수 있게 해주는 변수 
		...
		for(int i=0; i<studentList.length; i++) {
			...
			// student 객체가 가지고 있는 toString() 매서드를 호출해서 학생 정보를 
			// 문자열로 반환 받아 출력
			...
			// 해결방법 : 참조변수가 null인지 여부를 검사 studentList[i]
...
//			for(  : 배열에서 꺼낸 요소 하나를 저장할 변수 : 배열명 ) { }
	...
	private void updateStudent() {
		...
		boolean result = service.updateStudent(index,kor,eng,math); // 4개의 매개변수 실행 시킨후 boolean 실행
