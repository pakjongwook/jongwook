...
SELECT EMP_ID ,EMP_NAME ,DEPT_CODE, DEPT_TITLE -- DEPT_TITLE이 없다 EMPLOYEE 테이블에 
FROM EMPLOYEE;

SELECT EMP_ID ,EMP_NAME ,DEPT_CODE, DEPT_TITLE 
FROM EMPLOYEE
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID); -- [= 등가조인]  NULL 다르기 때문에 / 컬럼값이 같아야지만 결과값이 나옴
...
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE); -- 조인 하려는 컬럼명이 같을 때 
...
-->  *반드시 OUTER JOIN임을 명시해야 한다. EX) NULL 값을 포함시키고 싶다.

-- OUTER JOIN과 비교할 INNER JOIN 쿼리문
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE
/*INNER*/ JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID);
-- 기본 값이 INNER JOIN 이라서 생략
-- INNER JOIN : 연결하는 컬럼 값이 같은 행만 조회
...
-- ANSI 표준
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE LEFT /*OUTER*/ JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID); -- 하동운, 이오리 결과가 다른 것을 억지로 나타나도록 함.
-- **** 기존 INNER JOIN 결과에 (21행) *****
-- JOIN 구문 왼쪽에 작성된 EMPLOYEE 테이블의 행 중에서
-- JOIN 결과에 포함되지 않은 행 (2행) 들도 조회하도록 함.


-- 오라클 구문
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE , DEPARTMENT 
WHERE DEPT_CODE = DEPT_ID(+);   -- LEFT인데 방향은 반대로  RIGHT은 왼쪽 대신 컬럼명 옆에 
-- NULL = D1 ~ D9(+)
...
FROM EMPLOYEE RIGHT JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID); -- (결과에 나오지 않은)D3, D4, D7 (DEPARTMENT) 을 추가하도록
...
FROM EMPLOYEE , DEPARTMENT 
WHERE DEPT_CODE(+) = DEPT_ID;
-- WHERE (+)DEPT_CODE = DEPT_ID; [X]


-- 3) FULL [OUTER] JOIN   : 합치기에 사용한 두 테이블이 가진 모든 행을 결과에 포함
--   오라클은 왼쪽, 오른쪽 포함안되는 것들을 다 포함


-- ANSI 표준
SELECT EMP_NAME,DEPT_CODE , DEPT_ID ,DEPT_TITLE
FROM EMPLOYEE FULL JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID); 
-- INNER JOIN 에 포함 되지 않은 EMPLOYEE 하동운, 이요리  DEPARTMENT의 D3,D4,D7  포함되지 못한 행 모두를 결과에 포함
...
FROM EMPLOYEE E
JOIN SAL_GRADE S ON (SALARY BETWEEN MIN_SAL AND MAX_SAL);  -- A 이상 B 이하 
...
WHERE EMP.MANAGER_ID  = MGR.EMP_ID(+); -- 왼쪽
...
-- EMPLOYEE   : DEPT_CODE  CHER(2) D1 ~ D9 , NULL
-- DEPARTMENT : DEPT_ID    CHER(2) D1 ~ D9 
...
FROM EMPLOYEE 
JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE)
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID); -- 오류 
...
JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE)
WHERE JOB_NAME = '대리'
AND LOCAL_NAME LIKE 'ASIA%'; -- ASIA로 시작해서 뒤는 아무글자나
...
-- 사원명(EMPLOYEE), 주민번호(EMPLOYEE), 부서명(DEPARTMENT), 직급명(JOB)을 조회하시오.
SELECT EMP_ID, EMP_NO, DEPT_TITLE, JOB_NAME 
FROM EMPLOYEE 
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
JOIN JOB USING (JOB_CODE)
WHERE SUBSTR(EMP_NO,8,1)
AND SUBSTR(ENP_NO,1,2) LIKE '7%'
AND EMP_NAME = '전';

      
      
-- 2. 이름에 '형'자가 들어가는 직원들의 사번(JOB), 사원명, 부서명을 조회하시오.
SELECT ENP_ID, DEPT_TITLE
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
WHERE EMP_NAME = '형';
