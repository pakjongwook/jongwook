...
@Controller // 요청/응답 클래스 (제어) + bean 등록(Spring이 관리하는 객체)
...
// @RequestMapping(vaule="요청 주소", method=RequsestMethod.GET/POST) 
//-> GET / POST 방식을 구분
...
//	@PostMapping("/login")
	...
	public String login(Member inputMember, Model model
		...
		// -> 기본적으로 request scope
		// -> @SessionAttribute 어노테이션과 함께 사용 시 Sesison scope
		
		// @RequestParam(value="saveId",required=false) String saveId
		// -> name 속성 값이 "saveId"인 파라미터를 전달 받아 저장
		// -> required=false : 필수 아님(null 허용)
		// (주의)_ required 속성 미작성 시 기본값 true
		// 파라미터가 전달되지 않는 경우 주의
		
		// HttpServeltResponse resp : 서버 -> 클라이언트 응답 방법을 가지고 있는 객체
		
		
		// page, (1)request, (2)session, application
...
//		System.out.println(loginMember);
		...
		if(loginMember != null) { // 로그인 성공 시
			...
			/* redirect(재요청) 시 
			 * 기존 요청(request)이 사라지고 새로운 요청을 만들게되어
			 * 
...
//		session.invalidate(); // session 무효화
	...
	// 로그인 전용 화면 이동
	@GetMapping("/login")   // 공통된 주소 member는 앞부분 설정되어있기 때문에 설정 X
	public String login() {
	...
	public String signUp() {
		...
		// -> ViewResovler가 profix, suffix를 리턴 값 앞,뒤에 붙임
	...
	// 회원 가입 진행
	@PostMapping("/signUp")
	public String signUp(Member inputMember, String[] memberAddress, RedirectAttributes ra) {  // 0번, 1번, 2번의 서로 다른 index의 String 값을 받음
