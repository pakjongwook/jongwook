...
// -1 == 초기값 , 0 == 프로필 삭제(x버튼), 1== 새 이미지 업로드  [숫자 내가 임의로 지정]
...
if (imageInput != null) { // 화면에 imageInput이 있을 경우
    ...
    imageInput.addEventListener("change", e => {
        ...
        if (file != undefined && file.size > maxSize) { // 선택된 파일의 크기가 최대 크기를 초과한 경우
            alert("2MB 이하의 이미지를 선택해주세요.")
            imageInput.value = ""; // input type="file" 태그에 대입할 수 있는 value "" (빈칸) 뿐이다!
            deleteCheck = -1; // 취소 == 파일 없음 == 초기상태
        ...
        // 파일을 나타내는 URL(주소)을 result 속성으로 얻어올 수 있게 함.
        ...
        reader.onload = e => {
            ...
            deleteCheck = 1; //  1 : 새 이미지가 업로드가 되었을 때
        }
