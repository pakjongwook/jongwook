...
function selectNickname(email) {
    fetch("/selectNickname?email=" + email)
        // 쿼리 스트링 : 웹 개발에서 쿼리스트링은 URL 끝에 ?을 시작으로, 키=값 형태로 여러 쌍의 데이터를 전송하는 방법
...
btn1.addEventListener("click", () => {
    ...
    // fetch("요청주소",{}) GET 방식 요청 (파라미터를 쿼리스트링으로 추가)
...
// GET : url?k=v&k=v
// HTTP - header : JSON이야  body : JSON 받아서 명시  json : '{k:v, k:v}' -> 문자열로 바꿔주는 것
...
btn2.addEventListener('click', ()=>{
    ...
    //.then((resp)=>{return resp.json}) // 응답 객체를 매개변수로 얻어와 파싱
    .then( resp=>resp.json())  // json() 함수 꼭 써주기
    .then(member => { 
    console.log(member); 
...
btn3.addEventListener("click", () =>{
    ...
    .then( resp => resp.json() ) // 객체배열로 바뀜
    .then(memberList => { 
    console.log(memberList);

        result3.innerHTML = ""; // result3의 안에 있는 내용을 다 지워버리는 것
...
// 2. SockJS를 이용해서 클라이너트용 웹소켓 객체 생성
