...
function selectCommentList(){
    ...
    // REST (REpresentational(구분) State Transfer) API 
    // - 자원을 이름(주소)으로 구분(REpresentational) 하여
    //   자원의 상태(State)를 주고 받는 것(Transfer) ex) 물(자원) + 따라줘/버려줘/마셔줘 (상태) => 자원의 상태를 나타내는 것(Transfer)
    ...
    fetch("/comment?boardNo="+boardNo) // GET방식은 주소에 파라미터를 담아서 전송// 요청 주소 만 적으면 GET / post {method : header, body}
    .then(response => response.json()) // 응답 객체 -> 파싱 // JSON : 자바스크립트 객체 표기법 
...
addComment.addEventListener("click", e => { // 댓글 등록 버튼이 클릭이 되었을 때
    ...
    const data = {"commentContent": commentContent.value,
                    "memberNo" : loginMemberNo, 
                     "boardNo" : boardNo}; // JS 객체  게시글번호 == 게시글 번호


    fetch("/comment",{
        method : "POST",
        headers: {"Content-Type": "application/json"}, // 요청할때 전달되는 내용이 JSON 형식
