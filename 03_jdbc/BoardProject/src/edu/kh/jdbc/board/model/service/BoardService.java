...
import static edu.kh.jdbc.common.JDBCTemplate.*; // * 로 해야함 (종합)
...
public class BoardService {
	...
	public Board selectBoard(int input, int memberNo) throws Exception{
		...
		if(board != null) {
			...
			// *************************************************************
			// ** 해당 게시글에 대한 목록 조회 DAO 호출 **
			List<Comment> commentList = commentDao.selectCommentList(conn,input); // input 게시글 번호 
			
			// board에 댓글 목록 세팅
			board.setCommentList(commentList); // 두번째 단어 부터 대문자
			...
			if( board.getMemberNo() != memberNo) { // 작성자를 제외한 코드
	...
	public int insertBoard(String boardTitle, String boardContent, int memberNo) throws Exception {
		...
		// 다음 게시글 번호 생성 -> 4
		int boardNo = dao.nextBoardNo(conn); // boardNo => 4가 저장
